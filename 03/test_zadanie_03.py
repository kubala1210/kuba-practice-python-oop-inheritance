...
def test_blik(mock_blikpayment_data):
    assert mock_blikpayment_data.pay(
        100) == 'Zapłacono blikiem 100 zł (kod jednorazowy)'
...
def test_cash(mock_cashpayment_data):
    assert mock_cashpayment_data.pay(
        100) == 'Zapłacono gotówką 100 zł (proszę wydać resztę)'
